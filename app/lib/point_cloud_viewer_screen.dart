...
cube.Object createColoredMeshObject(Mesh mesh) {
  final cubeMesh = cube.Mesh();
  
  // Add vertices
  for (final v in mesh.vertices) {
    cubeMesh.vertices.add(vm.Vector3(v.x, v.y, v.z));
  }
  
  // Add vertex colors
  for (final v in mesh.vertices) {
    cubeMesh.colors.add(Color.fromRGBO(v.r, v.g, v.b, 1.0));
  }
  
  // Add texture coordinates (for completeness)
  for (final v in mesh.vertices) {
    cubeMesh.texcoords.add(Offset(v.u, v.v));
  }
  
  // Add triangles
  for (final t in mesh.triangles) {
    cubeMesh.indices.add(cube.Polygon(t.v1, t.v2, t.v3));
  }
  
  final obj = cube.Object(
    mesh: cubeMesh,
    backfaceCulling: true,
    lighting: false, // Disable lighting to show vertex colors directly
  );
...
class _PointCloudViewerScreenState extends State<PointCloudViewerScreen> {
  ...
  bool _showMesh = false; // Toggle between point cloud and mesh view
  
  // Flutter Cube scene for OBJ rendering (stored for camera control)
  // ignore: unused_field
  ...
  Future<void> _generatePointCloud() async {
    ...
    try {
      ...
      setState(() {
        ...
        _cubeScene = null; // Reset cube scene
        _showMesh = false;
  ...
  Future<void> _generateMesh() async {
    ...
    try {
      ...
      setState(() {
        ...
        _showMesh = true;
        _statusMessage =
            'Mesh: ${exportResult.vertexCount} vertices, ${exportResult.triangleCount} triangles';
      });
  ...
  Future<void> _shareMesh() async {
    if (_meshExportResult == null) return;

    try {
      final files = [
        XFile(_meshExportResult!.objPath),
        XFile(_meshExportResult!.mtlPath),
        XFile(_meshExportResult!.texturePath),
      ];
      ...
      await SharePlus.instance.share(
        ShareParams(
          ...
          text: 'Exported 3D mesh with ${_meshExportResult!.vertexCount} vertices',
        ),
      );
    } catch (e) {
      if (!mounted) return;
              ...
              children: [
                Expanded(
                  child: _pointCloud == null
                      ...
                      : _showMesh && _mesh != null
                          ? Container(
                              color: Colors.black,
                              child: cube.Cube(
                                onSceneCreated: (scene) {
                                  _cubeScene = scene;
                                  // Create mesh object with vertex colors directly
                                  final coloredMesh = createColoredMeshObject(_mesh!);
                                  scene.world.add(coloredMesh);
                                  // Set camera position
                                  scene.camera.zoom = 8.0;
                                },
                              ),
                            )
                          : GestureDetector(
                          ...
                          children: [
                            if (_mesh != null) ...[
                              ...
                              Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  const Text('Mesh', style: TextStyle(color: Colors.white, fontSize: 12)),
                                  Switch(
                                    value: _showMesh,
                                    onChanged: (value) {
                                      setState(() {
                              ...
                              ElevatedButton.icon(
                                ...
                                onPressed: _meshExportResult != null ? _shareMesh : null,
                                style: ElevatedButton.styleFrom(
